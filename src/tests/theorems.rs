...
fn test_random_collection_of_values_behave_correctly() {
    ...
    assert!(all_of_le.iter().filter(|v| **v).count() > 1, "Not all of these may be false");
    assert!(none_of_geq.iter().filter(|v| **v).count() > 1, "Not all of these may be false");
    assert!(any_of_le.iter().filter(|v| **v).count() > 1, "Not all of these may be false");
    ...
    assert!(all_of_le.iter().zip(none_of_geq.iter()).all(|(all_of_le, none_of_geq)| all_of_le == none_of_geq), "all_of and none_of give inconsistent results");
    // 2) all_of!({...}<rhs) == true implies that any_of!({...}< rhs) == true
    assert!(all_of_le.iter().zip(any_of_le.iter()).all(|(all_of_le, any_of_le)| if *all_of_le {*any_of_le==true}else{true}), "all_of and any_of give inconsistent results");
    // 3) none_of!({...}>= rhs) == true implies that any_of!({...}<rhs) == true
    assert!(none_of_geq.iter().zip(any_of_le.iter()).all(|(none_of_geq, any_of_le)| if *none_of_geq==true {*any_of_le==true}else{true}), "none_of and any_of give inconsistent results");
}
